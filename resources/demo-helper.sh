...
function p() {
  ...
  if [[ -z $TYPE_SPEED ]]; then
    # echo -en "$cmd" Did not work, always prints out the '-en' ...
    echo "$cmd"
  else
    # echo -en "$cmd" | pv -qL $[$TYPE_SPEED+(-2 + RANDOM%5)];  Did not work, always prints out the '-en' ...
    echo "$cmd" | pv -qL $[$TYPE_SPEED+(-2 + RANDOM%15)];
  fi
...
#################
# Some helper functions, ocm-demo specific
#################
